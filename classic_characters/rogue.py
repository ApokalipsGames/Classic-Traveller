# rogue.py
# module for Scouts characters

import random

# attributes
service = "Rogue"
muster_out_benefits = ["Low Psg", "+1 Soc", "Gun",
                       "Blade", "High Psg", "TAS Membership"]
muster_out_cash = [0, 0, 10000, 10000, 50000, 100000, 100000]

skills = [["Str", "Dex", "End", "Int", "Brawling", "Carousing"],  # pdt
          ["Blade Cbt", "Gun Cbt", "Demolition", "Vehicle", "Edu", "Vehicle"],  # sst
          ["Streetwise", "Forgery", "Bribery", "Carousing", "Liaison", "Ship's Boat"],  # aed
          ["Medical", "Bribery", "Forgery", "Computer", "Leader", "Jack-o-T"]]  # ed8+


# functions
def enlistment(upp):
    print("Attempting to join a criminal {} gang.".format(service))
    dm = 0
    if upp[5] <= 8:
        dm += 1
    if upp[2] >= 7:
...
def career(name, terms, age, reup, rank, upp):
    skills_learned = ["Streetwise"]
    print("Service skill learned: Streetwise")
    while reup == 'Y':
        terms += 1
        print("UPP:", upp)
        print("Term:", terms)
        ...
        if terms == 1:
            skills_available += 1
        # scouts have no commission or promotion
        # skills step
        ...
        for a in range(0, skills_available):
            ...
            print(skill)
            if skill == 'Str' or skill == 'Dex' or skill == 'End' or skill == 'Int' or skill == 'Edu':
