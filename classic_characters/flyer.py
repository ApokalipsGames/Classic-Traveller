# flyer.py
# module for Flyer (Air Force) characters

import random
import math

# attributes
service = "Aerospace Force"
ranks = ["", "Flying Officer", "Flight Lt", "Squadron Leader", "Wing Commander", "Group Captain", "Air Marshal"]
muster_out_benefits = ["Low Psg", "+1 Edu", "Weapon",
                       "Weapon", "High Psg", "Mid Psg", "+1 Soc"]
muster_out_cash = [2000, 5000, 10000, 10000, 10000, 20000, 30000]

skills = [["Str", "Dex", "End", "Gambling", "Brawling", "Carousing"],  # pdt
          ["Brawling", "Vacc Suit", "Gun Cbt", "Vehicle", "Vehicle", "Vehicle"],  # sst
          ["Aircraft", "Mechanical", "Electronic", "Gravitics", "Gun Cbt", "Survival"],  # aed
          ["Medical", "Leader", "Pilot", "Computer", "Jack-o-T", "Admin"]]  # ed8+


# functions
def enlistment(upp):
    print("Attempting to join the {}.".format(service))
    dm = 0
    if upp[0] >= 7:
        dm += 1
    if upp[1] >= 9:
...
def career(name, terms, age, reup, comm, rank, upp):
    skills_learned = ["Aircraft"]
    print("Service skill learned: Aircraft")
    while reup == 'Y':
        terms += 1
        print("UPP:", upp)
        print("Term:", terms)
