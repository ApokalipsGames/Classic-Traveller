...
muster_out_cash = [1000, 1000, 5000, 5000, 10000, 100000, 100000]

skills = [["Str", "Dex", "End", "Int", "Blade Cbt", "Gun Cbt"],  # pdt
          ["Blade Cbt", "Gun Cbt", "Survival", "Hunting", "Vehicle", "Hunting"],  # sst
          ["Gravitics", "Mechanical", "Electronic", "Computer", "Hunting", "Admin"],  # aed
          ["Medical", "Computer", "Hunting", "Leader", "Survival", "Admin"]]  # ed8+
...
def enlistment(upp):
    ...
    if roll_two_dice() + dm >= 9:
        print("Your application to become a {} was accepted!".format(service))
        return True
    else:
        print("Your application to become a {} was denied.".format(service))
        return False
...
def career(name, terms, age, reup, rank, upp):
    ...
    while reup == 'Y':
        ...
        if terms == 1:
            skills_available += 1
        # hunters have no commission or promotion
        # skills step
        ...
        if reup == 'Y':
            if reenlist():
                ...
                muster_out(terms, age, rank, upp, sorted_skills)
                reup = 'N'
        else:
            print("Mustering you out!")
            muster_out(name, name, terms, age, rank, upp, sorted_skills)
            reup = 'N'
